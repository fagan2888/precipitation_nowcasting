...
def fetchCheckpoints(p):
	...
	for f in allFiles:
		...
		num.append(int(subString))

	num = sorted(num)

	for i, n in enumerate(num):

		num[i] = 'checkpoint_' + str(n) +  '.pth.tar'

	return num[::-1]


def ensemblePredict(mList, x):

	for i, m in enumerate(mList):

		if i == 0:
			pred = makePrediction(m, x)
		else:

			pred += makePrediction(m, x)

	return pred/len(mList)
...
def main():
	...
	# Fetch the model checkpoints and put them in order
	checkpointPath = ''
	modelPaths = fetchCheckpoints(checkpointPath)

	# Number of model to include in the ensemble from end checkpoint backwards
	# 150 correspond to models back to iter around of 6000
	numModels = 150

	modelList = []
	for i in range(numModels):

		atmPath = checkpointPath + modelPaths[i]

		checkpoint = torch.load(atmPath)	
		modelList.append(resnet20())
		modelList[i].load_state_dict(checkpoint['state_dict'])

		print "Loading model {} and iter {}/{}".format(modelPaths[i], i, numModels)

	# Set in GPU and evaluate mode for each model
	for m in modelList:
		m.cuda()
		m.eval()
	...
	for d in dataFiles:
		...
		for i in range(atmData['nObservations'][0][0]):
			...
			# Make a prediction with ensemble
			pred = ensemblePredict(modelList, x)
